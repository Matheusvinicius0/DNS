...
async def get_profiles(): return ["all", "Padrão"]

@app.get("/api/stats")
async def get_stats(profile: str = 'all'):
...
async def get_blocklist_stats(profile: str = 'all'):
    logs = list(query_logs)
...
async def get_query_log(profile: str = 'all'): return {"logs": list(query_logs)}
...
async def doh_get(request: Request, name: str = Query(...), type: str = Query("A")):
    ...
    if blocking_list_url:
        ...
        log_query(name, "Bloqueado", client_ip, blocked_by=friendly_name)
    ...
    try:
        ...
        log_query(name, "Encaminhado", client_ip)
        result = {
        ...
        log_query(name, "Erro", client_ip, blocked_by=f"Resolver: {e}")
        return JSONResponse(status_code=500, content={"error": f"Erro ao resolver domínio: {e}"})

# Manipulador DoH para requisições POST
...
async def doh_post(request: Request):
    ...
    try:
        ...
        log_query(qname, "Encaminhado", client_ip)
